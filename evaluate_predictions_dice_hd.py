...
def compute_hd_avg_hd_95(mask_ref: np.ndarray, mask_pred: np.ndarray):    
    hausdorffcomputer = sitk.HausdorffDistanceImageFilter()    
    label_ref = sitk.Cast(sitk.GetImageFromArray(mask_ref[0, :, :, :].astype(np.float32)),sitk.sitkFloat32)
    label_pred = sitk.Cast(sitk.GetImageFromArray(mask_pred[0, :, :, :].astype(np.float32)),sitk.sitkFloat32)
    hausdorffcomputer.Execute(label_ref, label_pred)
    hd_avg = hausdorffcomputer.GetAverageHausdorffDistance()
    return hd_avg
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        if len(np.unique(mask_ref)) ==  1 or len(np.unique(mask_pred)) ==  1:
            hd_avg = np.nan
            print(prediction_file, r, hd_avg)
        else:
            hd_avg = compute_hd_avg_hd_95(mask_ref, mask_pred)
        ...
        results['metrics'][r]['avg_haudistance'] = hd_avg
    return results
...
if __name__ == '__main__':
    #name = 'test'
    folder_ref = r'C:\Users\MedbotAI\Desktop\nnunet-infer\labelsTs'
    folder_pred = rf"C:\Users\MedbotAI\Desktop\nnunet-infer\tmpoutput\fold_all\postprocessed"
    output_file = rf"C:\Users\MedbotAI\Desktop\nnunet-infer\summary/summary.json"
    ...
    summary = load_summary_json(output_file)

    # results = load_json("/data/data3/tmz/data/nnUNet_preprocessed/Dataset701_Hip/test_dataset_fingerprint.json")
    dataset_info = load_json(r"C:\Users\MedbotAI\Desktop\nnunet-infer\tmpoutput/dataset.json")
    
    dice_dict = {}
    hd_dict = {}
    
    for key, value in dataset_info['labels'].items():
        if value != 0:
            dice_dict[key] = []
            hd_dict[key] = []
    ...
    label_list = list(dataset_info['labels'].keys())[1:]
    
    import pandas as pd
    for idx in range(len(summary['metric_per_case'])):
        case_metric = summary['metric_per_case'][idx]['metrics']
        case_id.append(os.path.basename(summary['metric_per_case'][idx]['reference_file']))
        dice_dict[label_list[0]].append(case_metric[regions[0]]['Dice'])
        hd_dict[label_list[0]].append(case_metric[regions[0]]['avg_haudistance'])
        # dice_dict[label_list[1]].append(case_metric[regions[1]]['Dice'])
        # hd_dict[label_list[1]].append(case_metric[regions[1]]['avg_haudistance'])
        # dice_dict[label_list[2]].append(case_metric[regions[2]]['Dice'])
        # hd_dict[label_list[2]].append(case_metric[regions[2]]['avg_haudistance'])
        # spacing_.append(results['spacings'][idx])
        # print(idx)

    data ={
        ('Case', ''): case_id,
        (label_list[0], 'Dice'): dice_dict[label_list[0]],
        (label_list[0], 'HD'): hd_dict[label_list[0]],
        # (label_list[1], 'Dice'): dice_dict[label_list[1]],
        # (label_list[1], 'HD'): hd_dict[label_list[1]],
        # (label_list[2], 'Dice'): dice_dict[label_list[2]],
        # (label_list[2], 'HD'): hd_dict[label_list[2]],
        # ('Spacing', '[z, y, x]'): spacing_
        }
    ...
    df.columns = columns
    df.to_excel('./summary.xlsx', index=True)
